...
def save_fitting_image(file, name, old_path=None):
    ...
    safe_name = re.sub(r'[^\w\-]', '', name.replace(' ', '_'), flags=re.UNICODE)
    if not safe_name:
        safe_name = 'image'
...
def create_kitchen_folders(kitchen_name):
    ...
    try:
        base_photo_path = os.path.join(current_app.static_folder, 'images', 'photo')
        base_schemes_path = os.path.join(current_app.static_folder, 'images', 'schemes')
